...
FigureContainer::FigureContainer(const FigureContainer& arr) {//конструктор копировани€
...
FigureContainer& ::FigureContainer::operator=(FigureContainer arr) {//перегрузка оператора присваивани€
    this->swap(arr);
    return *this;
}
...
int FigureContainer::get_size() const {//реализаци€ метода получени€ пользовательского размера массива
    return user_size;
}
...
int Figures::min_square(const FigureContainer& figures) {//функци€ дл€ вычислени€ минимальной площади
    int last_index = -1;
    int min_value = 0;
